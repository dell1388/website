...
Anything this project ships lives under `garrison/` or `radar/` so neither
can collide with it. An earlier layout used a top-level `js/` folder;
uploading that would have overwritten Aetherfall's client and broken the
wizard game. `radar/` has its own `style.css` and its own `js/` tree
(`radar/js/...`), entirely separate from both the root `/js/` and
`garrison/js/`.
