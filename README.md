...
The front page is a small top-down tank game. You drive with **WASD**, aim the
turret with the mouse (or the arrow keys), and put three shells through the
target in a room to open that room's page.
...
js/
  ...
  ui/               HUD, minimap, title/pause/breach overlays
...
and fire. Every destination is also a plain link in the menu, on the title
screen and in `classic.html`, so nobody is forced to play to get somewhere.
